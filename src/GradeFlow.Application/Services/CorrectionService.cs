...
public interface ICorrectionService
{
    ...
    Task<CorrectionResponse?> CorrectQuestionAsync(Guid submissionId, Guid questionId, CancellationToken cancellationToken = default);
}
    ...
    IEnumerable<ICorrectionStrategy> strategies) : ICorrectionService
{
    ...
    public async Task<CorrectionResponse?> CorrectQuestionAsync(
        Guid submissionId,
        Guid questionId,
        CancellationToken cancellationToken = default)
    {
        ...
        CorrectAnswer(answer, submissionRepository);

        submission.FinalScore = submission.StudentAnswers.Sum(x => x.ScoreAwarded);
        submission.Status = submission.StudentAnswers.All(x => x.Feedback is not null)
            ? SubmissionStatus.Corrected
            : SubmissionStatus.Pending;
        submission.CorrectedAt = DateTime.UtcNow;

        await submissionRepository.SaveChangesAsync(cancellationToken);
        return Map(submission, questionId);
    }

    private void CorrectAnswer(StudentAnswer answer, ISubmissionRepository submissionRepository)
    {
        if (answer.Question?.AnswerKey is null)
        {
            throw new ValidationException("Every answered question must have an answer key.");
    ...
    private static CorrectionResponse Map(Submission submission, Guid? questionId = null)
        => new(
            submission.Id,
            ...
            submission.StudentAnswers
                .Where(x => questionId is null || x.QuestionId == questionId)
